...
PlayerLastStand( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	// bleed out timer
	self thread laststand_bleedout( GetDvarFloat( BleedOutTime ) );
...
laststand_take_player_weapons( solo )
{
	...
	*/
	...
	for( i = 0; i < self.weaponInventory.size; i++ )
	{
		...
		if( solo && !self hasWeapon("ray_gun") && !self hasWeapon("ray_gun_upgraded") )
		{
			self.laststandpistol = level.startguns[pis_num] + "_upgraded";
		}

		else if( !self hasWeapon("ray_gun") && !self hasWeapon("ray_gun_upgraded") && !self hasWeapon("zombie_sw_357") && !self hasWeapon("zombie_sw_357_upgraded")
		&& !self hasWeapon("zombie_colt_upgraded") && !self hasWeapon("zombie_tokarev_upgraded") && !self hasWeapon("zombie_nambu_upgraded") && !self hasWeapon("zombie_walther_upgraded") )
		{
			if(isDefined(self.last_stand_weapon))
			{
				self.laststandpistol = self.last_stand_weapon;
		...
		else if( self hasWeapon("ray_gun_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "ray_gun_upgraded";
		}

		else if( self hasWeapon("ray_gun") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "ray_gun";
		}

		else if( self hasWeapon("zombie_sw_357_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "zombie_sw_357_upgraded";
		}

		else if( self hasWeapon("zombie_sw_357") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "zombie_sw_357";
		}

		else if( self hasWeapon("zombie_colt_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "zombie_colt_upgraded";
		}

		else if( self hasWeapon("zombie_tokarev_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "zombie_tokarev_upgraded";
		}

		else if( self hasWeapon("zombie_nambu_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
			self.laststandpistol = "zombie_nambu_upgraded";
		}

		else if( self hasWeapon("zombie_walther_upgraded") && !IsDefined( self.laststandpistol ) ) 
		{
...
say_reviving_vo()
{
	if( level.players_can_talk == false )
		return;
