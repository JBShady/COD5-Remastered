...
start_carpenter( origin )
{
	...
	while(true)
	{
		...
		while(1)
		{
			...
			windows thread maps\_zombiemode_blockers::replace_chunk( chunk, true );
...
powerup_vo(type)
{
	...
	{
		case "nuke":
				sound_to_play = "vox_powerup_nuke_" + rand;
				waittime = 1.485;
				break;
		case "insta_kill":
		...
		case "full_ammo":
				...
				break;
		case "double_points":
				rand_doub = randomintrange(0,3);
				sound_to_play = "vox_powerup_double_" + rand_doub;
				break; 		
		case "carpenter":
				sound_to_play = "vox_powerup_carp_" + rand;
				waittime = 0.85;
				break;
	}
